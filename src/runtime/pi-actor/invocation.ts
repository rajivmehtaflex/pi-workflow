...
export function resolvePiInvocation(options: PiInvocationOptions): PiInvocation {
  ...
  if (options.systemPromptPath !== undefined) args.push("--append-system-prompt", options.systemPromptPath);
  args.push(options.prompt);
  return {
    executable: options.runningScript === undefined ? options.fallbackExecutable ?? "pi" : process.execPath,
    args,
  };
}
