...
function record(value: unknown): Record<string, unknown> {
  if (typeof value !== "object" || value === null || Array.isArray(value))
    throw new WorkflowProtocolError("Message must be a JSON object");
...
export function parseChildMessage(line: string): ChildMessage {
  ...
  if (kind === "create-actor") {
    return {
      ...
      ...(stringField(input, "name", false) === undefined
        ? {}
        : { name: stringField(input, "name", false) }),
      ...(input.persona === undefined ? {} : { persona: input.persona }),
    };
  }
  if (kind === "request") {
    const type = stringField(input, "type");
    if (!["ask", "world-read", "world-run", "publish-artifact", "escalate"].includes(type!))
      throw new WorkflowProtocolError(`Unknown request type: ${type}`, "UnknownRequest");
    return {
      kind,
      id: stringField(input, "id")!,
      type: type as ChildRequestMessage["type"],
      siteId: stringField(input, "siteId")!,
      ...(stringField(input, "actor", false) === undefined
        ? {}
        : { actor: stringField(input, "actor", false) }),
      ...(stringField(input, "instructions", false) === undefined
        ? {}
        : { instructions: stringField(input, "instructions", false) }),
      ...(stringField(input, "op", false) === undefined
        ? {}
        : { op: stringField(input, "op", false) }),
  ...
  if (kind === "event") {
    const type = stringField(input, "type");
    if (
      !["phase-entered", "log", "report", "declare-artifact", "artifact-published"].includes(type!)
    )
      throw new WorkflowProtocolError(`Unknown event type: ${type}`, "UnknownEvent");
    return { ...input, kind, type: type as ChildEventMessage["type"] } as ChildEventMessage;
  }
  if (kind === "complete") {
    if (typeof input.ok !== "boolean")
      throw new WorkflowProtocolError("Complete message requires boolean ok");
    if (input.error !== undefined) {
      const error = record(input.error);
      if (typeof error.code !== "string" || typeof error.message !== "string")
        throw new WorkflowProtocolError("Complete error is malformed");
    }
    return {
      kind,
      ok: input.ok,
      ...(input.value === undefined ? {} : { value: input.value }),
      ...(input.error === undefined ? {} : { error: input.error as WireError }),
    };
  }
  ...
  push(chunk: Uint8Array | string): ChildMessage[] {
    ...
    while (newline >= 0) {
      ...
      if (Buffer.byteLength(line, "utf8") > this.options.maxLineBytes)
        throw new WorkflowProtocolError("NDJSON line exceeds limit", "LineTooLarge");
    ...
    if (this.buffer.trim() !== "")
      throw new WorkflowProtocolError("Child ended with a partial NDJSON line", "PartialLine");
  }
}
