...
/* oxlint-disable max-lines -- repository methods share one transaction boundary. */
...
function runFromRow(row: Row): RunRecord {
  return {
    ...
    ...(typeof row.stop_reason === "string"
      ? { stopReason: row.stop_reason as RunRecord["stopReason"] }
      : {}),
...
function actorFromRow(row: Row): ActorRecord {
  return {
    ...
    ...(row.session_message_count === null
      ? {}
...
export class WorkflowRepository implements JournalStorePort {
  ...
  updateRunStatus(runId: string, status: RunStatus, settlement?: RunSettlementRecord): void {
    this.transaction(() => {
      ...
      if (status === "pending" || status === "running") {
        this.db
          .prepare(
            "UPDATE workflow_runs SET failure_json = NULL, result_json = NULL, stop_reason = NULL, superseded_by = NULL WHERE run_id = ?",
          )
          .run(runId);
      }
    });
  ...
  updateRunUsage(runId: string, spentTokens: number): void {
    const result = this.db
      .prepare("UPDATE workflow_runs SET spent_tokens = ?, updated_at = ? WHERE run_id = ?")
  ...
  putActor(record: ActorRecord): void {
    ...
    `)
      .run(
        ...
        record.ordinal,
        record.name ?? null,
        encode(record.persona),
        record.resolvedModel ?? null,
        record.sessionId ?? null,
        record.sessionPath ?? null,
        record.sessionMessageCount ?? null,
  ...
  getActor(runId: string, siteId: string, ordinal: number): ActorRecord | undefined {
    const row = this.db
      .prepare("SELECT * FROM workflow_actors WHERE run_id = ? AND site_id = ? AND ordinal = ?")
      .get(runId, siteId, ordinal) as Row | undefined;
  ...
  listActors(runId: string): ActorRecord[] {
    return (
      this.db
        .prepare("SELECT * FROM workflow_actors WHERE run_id = ? ORDER BY site_id, ordinal")
        .all(runId) as Row[]
    ).map(actorFromRow);
  }
  ...
  getNode(runId: string, siteId: string, ordinal: number): NodeRecord | undefined {
    const row = this.db
      .prepare("SELECT * FROM workflow_nodes WHERE run_id = ? AND site_id = ? AND ordinal = ?")
      .get(runId, siteId, ordinal) as Row | undefined;
  ...
  listNodes(runId: string): NodeRecord[] {
    return (
      this.db
        .prepare("SELECT * FROM workflow_nodes WHERE run_id = ? ORDER BY site_id, ordinal")
        .all(runId) as Row[]
    ).map(nodeFromRow);
  }
  ...
  appendEvent(runId: string, event: RunEvent): StoredEvent {
    return this.transaction(() => {
      if (this.getRun(runId) === undefined)
        throw new Error(`workflow journal: unknown run ${runId}`);
      const row = this.db
        .prepare(
          "SELECT COALESCE(MAX(sequence), 0) + 1 AS sequence FROM workflow_journal WHERE run_id = ?",
        )
        .get(runId) as Row;
      ...
      this.db
        .prepare(
          "INSERT INTO workflow_journal (run_id, sequence, event_type, payload_json, time_created) VALUES (?, ?, ?, ?, ?)",
        )
        .run(runId, sequence, event.type, JSON.stringify(event), timeCreated);
  ...
  listEvents(runId: string, options: ListEventsOptions = {}): StoredEvent[] {
    ...
    const rows =
      limit === undefined
        ? this.db
            .prepare(
              "SELECT sequence, payload_json, time_created FROM workflow_journal WHERE run_id = ? AND sequence > ? ORDER BY sequence",
            )
            .all(runId, after)
        : this.db
            .prepare(
              "SELECT sequence, payload_json, time_created FROM workflow_journal WHERE run_id = ? AND sequence > ? ORDER BY sequence LIMIT ?",
            )
            .all(runId, after, limit);
    return (rows as Row[]).map((row) => ({
      sequence: Number(row.sequence),
      event: decode(row.payload_json, {} as RunEvent),
  ...
  insertArtifactVersion(runId: string, artifact: ArtifactInsert): ArtifactVersionRecord {
    return this.transaction(() => {
      const row = this.db
        .prepare(
          "SELECT COALESCE(MAX(version), 0) + 1 AS version FROM workflow_artifacts WHERE run_id = ? AND artifact_id = ?",
        )
        .get(runId, artifact.id) as Row;
      ...
      return {
        id: artifact.id,
        version,
        kind: artifact.kind as ArtifactVersionRecord["kind"],
        ...
        ...(artifact.primary ? { primary: true as const } : {}),
        createdAt,
      };
    });
  }
  ...
  listArtifactVersions(runId: string, artifactId: string): ArtifactVersionRecord[] {
    return (
      this.db
        .prepare(
          "SELECT * FROM workflow_artifacts WHERE run_id = ? AND artifact_id = ? ORDER BY version",
        )
        .all(runId, artifactId) as Row[]
    ).map((row) => ({
      id: String(row.artifact_id),
      version: Number(row.version),
      kind: row.kind as ArtifactVersionRecord["kind"],
      ...
      ...(typeof row.source_path === "string" ? { sourcePath: row.source_path } : {}),
      ...(row.spec_json === null ? {} : { spec: decode(row.spec_json, undefined) }),
      ...(row.primary_artifact ? { primary: true as const } : {}),
      createdAt: Number(row.created_at),
    }));
  }
  ...
  listRuns(workspaceKey: string, limit = 20): RunRecord[] {
    return (
      this.db
        .prepare(
          "SELECT * FROM workflow_runs WHERE workspace_key = ? ORDER BY updated_at DESC LIMIT ?",
        )
        .all(workspaceKey, Math.max(1, Math.min(100, Math.floor(limit)))) as Row[]
    ).map(runFromRow);
  }
  ...
  listNonTerminalRuns(workspaceKey: string): RunRecord[] {
    return (
      this.db
        .prepare(
          "SELECT * FROM workflow_runs WHERE workspace_key = ? AND status IN ('pending','running') ORDER BY updated_at DESC",
        )
  ...
  getEscalation(qid: string): EscalationRecord | undefined {
    const row = this.db.prepare("SELECT * FROM workflow_escalations WHERE qid = ?").get(qid) as
      | Row
      | undefined;
  ...
  updateEscalation(qid: string, status: EscalationStatus, answer?: string): EscalationRecord {
    const result = this.db
      .prepare(
        "UPDATE workflow_escalations SET status = ?, answer = ?, resolved_at = ? WHERE qid = ? AND status = 'pending'",
      )
      .run(status, answer ?? null, status === "pending" ? null : Date.now(), qid);
    if (result.changes !== 1)
      throw new Error(`workflow journal: escalation is missing or already settled: ${qid}`);
  ...
  listPendingEscalations(runId: string): EscalationRecord[] {
    const rows = this.db
      .prepare(
        "SELECT * FROM workflow_escalations WHERE run_id = ? AND status = 'pending' ORDER BY asked_at, qid",
      )
      .all(runId) as Row[];
    return rows
      .map((row) => this.getEscalation(String(row.qid))!)
      .filter((record): record is EscalationRecord => record !== undefined);
  }

  saveWorkflow(record: SavedWorkflowRecord): void {
    this.db
      .prepare(`INSERT INTO workflow_saved_workflows (scope, name, source_text, script_hash, args_schema_json, updated_at)
      VALUES (?, ?, ?, ?, ?, ?) ON CONFLICT(scope, name) DO UPDATE SET source_text = excluded.source_text,
      script_hash = excluded.script_hash, args_schema_json = excluded.args_schema_json, updated_at = excluded.updated_at`)
      .run(
        record.scope,
        record.name,
        record.sourceText,
        record.scriptHash,
        encode(record.argsSchema),
        record.updatedAt,
      );
  }
  ...
  listSavedWorkflows(scope?: SavedWorkflowRecord["scope"]): SavedWorkflowRecord[] {
    const rows =
      scope === undefined
        ...
        : this.db
            .prepare("SELECT * FROM workflow_saved_workflows WHERE scope = ? ORDER BY name")
            .all(scope);
    return (rows as Row[]).map((row) => ({
      scope: row.scope as SavedWorkflowRecord["scope"],
      name: String(row.name),
      sourceText: String(row.source_text),
      scriptHash: String(row.script_hash),
      ...(row.args_schema_json === null
        ? {}
        : { argsSchema: decode(row.args_schema_json, undefined) }),
      updatedAt: Number(row.updated_at),
    }));
  }
}
