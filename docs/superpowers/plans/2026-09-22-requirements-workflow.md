...
- [x] Write SQLite tests: migrate version 1 without losing runs; persist/reopen requests;
...
- [x] Run `pnpm exec vitest run tests/requirements-storage.test.ts`; confirm missing behavior.
- [x] Add migration version 2 with `workflow_requests` and `workflow_request_attempts`;
...
- [x] Pass migration, duplicate-input, and UTF-8 limit tests; commit this task.
...
- [x] Add subprocess fixtures returning valid envelopes, malformed JSON, oversized output,
...
- [x] Run `pnpm exec vitest run tests/requirements-generator.test.ts tests/actor-driver.test.ts`.
- [x] Implement envelope parsing with TypeBox and no additional properties. Allow exactly
...
- [x] Apply explicit no-tools/no-extensions/no-skills/no-prompt-templates options supported
...
- [x] Write tests with real compiler: valid first candidate; invalid candidate followed by
...
- [x] Run `pnpm exec vitest run tests/requirements-coordinator.test.ts` and observe failures.
- [x] Implement the loop: persist generating and increment attempt; await candidate under
...
- [x] Verify exhausted repairs never create a run; Unicode input limits, timeout, duplicate
...
- [x] Test two admissions for one request yield one persisted run; rollback does not leave
...
- [x] Run `pnpm exec vitest run tests/requirements-admission.test.ts tests/service-resume.test.ts`.
- [x] Extract run admission from current createWorkflow only as needed: compile, create run,
...
- [x] Reconcile request states against existing run states on session start. Mark unfinished
...
- [x] Test `/workflow auto` with spaces, embedded JSON, multiline requirements, and `--`;
...
- [x] Run `pnpm exec vitest run tests/requirements-surface.test.ts tests/commands.test.ts tests/extension-smoke.test.ts`.
- [x] Register create_workflow_from_requirements with the task 1 input schema. Return a
...
- [x] Show generating, repairing, running, and terminal states. Observe settlement without
...
- [x] Add a provider-free integration case using the real compiler, SQLite, coordinator,
...
- [x] Run `pnpm test`, `pnpm typecheck`, `pnpm build`, `pnpm lint`, `pnpm fmt:check`.
- [x] Verify a fresh disposable source checkout using `npm install --omit=dev`; load its
  generated dist with the available Pi 0.87 host; no Pi 0.86 host was installed. Confirm
  tool registration and command handling in an isolated PI_CODING_AGENT_DIR. Do not reuse
  developer node_modules.
- [x] Check provider-backed validation: no recognized provider credential was available,
  so live provider validation remains explicitly unverified.
- [x] Document a single prompt and `/workflow auto` example, optional preview, status,
