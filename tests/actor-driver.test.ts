...
describe("Boundary-B Pi actor process", () => {
  ...
  it("derives a run-scoped actor session path", async () => {
    ...
    const first = actorSessionPath({ cwd, workspaceKey: "workspace-1", runId: "run/1", actor: { siteId: "actor#1", ordinal: 1 } });
    const second = actorSessionPath({ cwd, workspaceKey: "workspace-1", runId: "run/1", actor: { siteId: "actor#2", ordinal: 1 } });
  ...
  it("runs a fake Pi child, captures final JSON, usage, and progress", async () => {
    ...
    const result = await spawnPiActorTurn({
      ...
      executableArgs: [fake],
      sessionPath: actorSessionPath({ cwd, workspaceKey: "workspace-1", runId: "run-1", actor: { siteId: "actor#1", ordinal: 1 } }),
      prompt: "inspect",
  ...
  it("stops malformed or hanging children and ignores late events", async () => {
    ...
    await expect(spawnPiActorTurn({ cwd, executable: process.execPath, executableArgs: [malformed], sessionPath: join(cwd, "a.jsonl"), prompt: "x" })).resolves.toMatchObject({ status: "errored" });

    const hanging = await fakePi(cwd, `
    ...
    `);
    const result = await spawnPiActorTurn({ cwd, executable: process.execPath, executableArgs: [hanging], sessionPath: join(cwd, "b.jsonl"), prompt: "x", timeoutMs: 200 });
