...
describe("SQLite workflow journal", () => {
  ...
  it("persists runs, actors, nodes, usage, script hashes, and terminal settlement atomically", async () => {
    ...
    journal.putNode({ runId: "run-1", siteId: "ask#1", ordinal: 1, kind: "ask", inputHash: "input-1", status: "running" });
  ...
  it("rejects duplicate actor and node identities", async () => {
    ...
    expect(() => journal.putNode({ runId: "run-1", siteId: "ask#1", ordinal: 1, kind: "ask", inputHash: "input-2", status: "running" })).toThrow();
    database.close();
  });
  ...
  it("allocates journal sequences and paginates after a cursor in SQL order", async () => {
    ...
    expect(journal.listEvents("run-1", { afterSequence: 1, limit: 1 }).map((event) => event.event)).toEqual([events[1]]);
  ...
  it("versions immutable artifacts and preserves resume mismatch data", async () => {
    ...
    const journal = new WorkflowJournal(database.db);
    journal.createRun(runRecord());
    const first = journal.insertArtifactVersion("run-1", { id: "report", kind: "markdown", bytes: 4, sha256: "a" });
    const second = journal.insertArtifactVersion("run-1", { id: "report", kind: "markdown", bytes: 5, sha256: "b" });
